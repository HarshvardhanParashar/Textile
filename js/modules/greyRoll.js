...
export function setupGreyHandlers() {
    ...
    const applyInwardAutoFill = async (beamNo = '', loomNo = '', lookupBy = '') => {
        ...
        const normalizedLoom = (loomNo || '').trim();

        if (!normalizedBeam && !normalizedLoom) return;

        try {
            let record = null;
            if (lookupBy === 'loom' && normalizedLoom) {
                record = await sendRequest(`inward/loom/${encodeURIComponent(normalizedLoom)}`);
            } else if (normalizedBeam) {
                record = await sendRequest(`inward/beam/${encodeURIComponent(normalizedBeam)}`);
            }
            if (!record && normalizedLoom) {
                record = await sendRequest(`inward/loom/${encodeURIComponent(normalizedLoom)}`);
            }

            if (!record) return;

            const beamField = document.getElementById('gr-beam');
            const loomField = document.getElementById('gr-loom');
            if (beamField && (lookupBy === 'loom' || !beamField.value.trim())) {
                beamField.value = record.id || normalizedBeam;
            }
            ...
            } else if (normalizedLoom) {
                showToast(`⚠️ Loom #${normalizedLoom} not found in Inward Stock.`, 'error');
            }
        }
    };
    ...
    beamInput?.addEventListener('change', async (e) => {
        ...
        await applyInwardAutoFill(beamNo, document.getElementById('gr-loom')?.value || '', 'beam');
    });

    const loomInput = document.getElementById('gr-loom');
...
function renderGreyTableUI(rolls) {
    ...
    const pagination = document.getElementById('grey-pagination');
    if (pagination) {
        pagination.innerHTML = `
            <button class="btn btn-outline btn-sm" id="grey-prev" ${showAllDates || selectedDate || greyDatePage === 0 ? 'disabled' : ''}>Previous</button>
            <span>${showAllDates ? 'All dates' : selectedDate ? `Date filter &middot; ${dateLabel}` : `Page ${dates.length ? greyDatePage + 1 : 0} of ${dates.length} &middot; ${dateLabel}`}</span>
            <button class="btn btn-outline btn-sm" id="grey-next" ${showAllDates || selectedDate || greyDatePage >= dates.length - 1 ? 'disabled' : ''}>Next</button>
        `;
        pagination.querySelector('#grey-prev')?.addEventListener('click', () => {
            greyDatePage -= 1;
            renderGreyTableUI(cachedGreyRolls);
        });
        pagination.querySelector('#grey-next')?.addEventListener('click', () => {
            greyDatePage += 1;
            renderGreyTableUI(cachedGreyRolls);
        });
    }
